/*
* NEW BrowserCompAPI version 1.0.6.6
...
* 5. MAM_BC.js - containing MAM functions implementation and objects
* 6. SmartBar_BC.js - containing SmartBar function implementation and objects
* 7. MAM_functions.js - containing SmartBar function implementation and objects
*/
if (window.isBcApiInitialized) {
// we allow only one instance of BCAPI
throw new Error('BCAPI already initialized');
}
window.isBcApiInitialized = true;
...
window.conduitWindowName = window.conduitWindowName || window.name; // Used instead of window.name, defaults to window.name
...
embed.id = 'pluginObj';
embed.type = 'ConduitChromeApi';
embed.setAttribute('style', 'height:0; visibility: hidden; position: fixed;');
...
try {
context = JSON.parse(id);
if (typeof context != 'object') {
return false;
}
globalContext = context;
window.name = context.name;
conduitWindowName = context.name;
} catch (e) {
...
isFF: function () {
return (navigator.userAgent.indexOf("Firefox") != -1 && !navigator.userAgent.match(/Trident/));
},
...
ExtractParam: function (paramName) {
var url = window.location.href;
var match = url.toLowerCase().match('[?&]' + paramName + '=([^&]+)');
...
var myconduitAppData = checkLocalStorageAvailbility() ? localStorage.getItem("smartbarBcApi") : undefined;
...
if (!isNotSmartbar() && _BCAPIHelper.isFF() && (window != window.top) && !getContext(window.conduitWindowName) && typeof (abstractionlayer) === "undefined") {
var lastLoadNumber = checkLocalStorageAvailbility() ? localStorage.getItem("loadNumber") : undefined;
...
else if (_BCAPIHelper.isFF()) {
if (checkLocalStorageAvailbility()) {
localStorage.removeItem("loadNumber");
}
}
if (navigator.userAgent.indexOf("Chrome") != -1 && (document.location.href.indexOf("#mam_extension") != -1 || document.location.href.indexOf("#mam_app") != -1 || document.location.href.indexOf("#mam_extension_gadget") != -1)) {
compatibleBcApi = new MAM_BcApi();
} else if (window.conduitSmartBarIndicator || !isNotSmartbar() && (getContext(window.conduitWindowName) || /___requireStorage/.test(document.location.href) || myconduitAppData || typeof (abstractionlayer) !== "undefined")) {
compatibleBcApi = new SmartBar_BcApi();
}
else if (_BCAPIHelper.isFF()) {
compatibleBcApi = new FF_BcApi();
}
else if (_BCAPIHelper.isIE()) {
...
BcApiObj.StoreKey(key, value, callback);
};
function RetrieveKey(key, callback) {
return BcApiObj.RetrieveKey(key, callback);
};
function DeleteKey(key, callback) {
BcApiObj.DeleteKey(key, callback);
};
function StoreGlobalKey(key, value, callback) {
BcApiObj.StoreGlobalKey(key, value, callback);
};
function RetrieveGlobalKey(key, callback) {
return BcApiObj.RetrieveGlobalKey(key, callback);
};
function DeleteGlobalKey(key, callback) {
BcApiObj.DeleteGlobalKey(key, callback);
};
function StoreGlobalKeyAsString(key, value) {
BcApiObj.StoreGlobalKeyAsString(key, value);
};
function RetrieveGlobalKeyAsString(key) {
return BcApiObj.RetrieveGlobalKeyAsString(key);
};
function SetRegistryKey(key, value) {
return BcApiObj.SetRegistryKey(key, value);
};
function GetRegistryKey(key) {
return BcApiObj.GetRegistryKey(key);
};
function DeleteGlobalKeyAsString(key) {
BcApiObj.DeleteGlobalKeyAsString(key);
};
...
function GetMainFrameUrl(callback) {//TODO: find out what is " if (!myIsAllowNonPrivacy) SetLastError(1); "
return BcApiObj.GetMainFrameUrl(callback);
};
...
function SaveFile(strFileName, strFileContent, callback) {
return BcApiObj.SaveFile(strFileName, strFileContent, callback);
};
function LoadFile(strFileName, callback) {
return BcApiObj.LoadFile(strFileName, callback);
};
function DeleteFile(strFileName, callback) {
return BcApiObj.DeleteFile(strFileName, callback);
};
...
function NavigateInNewTab2(url) {
return BcApiObj.NavigateInNewTab2(url);
};
...
function ExecuteScriptInFrame(frameId, script, callback) {
return BcApiObj.ExecuteScriptInFrame(frameId, script, callback);
};
function __GeneralResponse(hashKey, p1, p2, p3, p4, p5, p6) {
_BCAPIHelper.callbacksHash[hashKey](p1, p2, p3, p4, p5, p6);
delete _BCAPIHelper.callbacksHash[hashKey];
};
function AddWebApp(guid, info, callback) {
return BcApiObj.AddWebApp(guid, info, callback);
};
function RemoveWebApp(guid, callback) {
return BcApiObj.RemoveWebApp(guid, callback);
};
function TriggerWebAppEvent(guid, info, callback) {
return BcApiObj.TriggerWebAppEvent(guid, info, callback);
};
function OnToolbarReady(callback) {
return BcApiObj.OnToolbarReady(callback);
};
function CreateNestedContext(frameId, callback) {
return BcApiObj.CreateNestedContext(frameId, callback);
};
function GetToolbarBornDate(callback) {
return BcApiObj.GetToolbarBornDate(callback);
};
function GetLoginDate(callback) {
return BcApiObj.GetLoginDate(callback);
};
function GetInstallDate(callback) {
return BcApiObj.GetInstallDate(callback);
};
function ShowToolbar(callback) {
return BcApiObj.ShowToolbar(callback);
};
function HideToolbar(callback) {
return BcApiObj.HideToolbar(callback);
};
function GetNavHistory(requestObj, callback) {
return BcApiObj.GetNavHistory(requestObj, callback);
};
function GetBookmarkList(requestObj, callback) {
return BcApiObj.GetBookmarkList(requestObj, callback);
};
function GetBookmark(requestObj, callback) {
return BcApiObj.GetBookmark(requestObj, callback);
};
function AddApp(guid, appName, callback) {
return BcApiObj.AddApp(guid, appName, callback);
};
function OnWebAppInjection(callback) {
if (BcApiObj.OnWebAppInjection)
BcApiObj.OnWebAppInjection(callback);
};
function GetMachineId(callback) {
return BcApiObj.GetMachineId(callback);
};
...
API.DOMContentLoaded = function (extraData) {
};
})();
//#endregion
...
dataStruc.toolbarInfo.bornDate = '';
...
if (commandData.indexOf('storeGlobalKeyAsString') == 0) {
HiddenCommandDiv.setAttribute("text", commandData);
}
...
var extraDataOnLoadFillDataStruc = null;
this.onLoadFillDataStruc = function (extraData) {
if (extraData) {
extraDataOnLoadFillDataStruc = extraData;
}
var me = this;
var mini_me = this;
var functionName = "getOnLoadData";
var divReturnId = "returnOnLoadData";
this.localStore = checkLocalStorageAvailbility() ? window.localStorage : undefined;
...
if (objInnerText && objInnerText.version) {
if (window.isInvokeEBNavigateCompleteFired) return;
window.isInvokeEBNavigateCompleteFired = true;
window.invokeEBNavigateComplete();
}
dataStruc.toolbarInfo.originalCTID = objInnerText.originalCTID;
dataStruc.toolbarInfo.CTID = objInnerText.CTID;
dataStruc.toolbarInfo.bornDate = objInnerText.returnedToolbarBornDate;
}
...
if ('isExtensionInstalledReturned' in objInnerText) {
isExtensionInstalledReturned(objInnerText.result, objInnerText.hashKey);
}
...
refreshGlobalKeys();
refreshGlobalKeys(true);
checkIfSearchWasMade();
setTargetsLinksForHashes();
removeExtraQueryStringHash();
window.invokeEBNavigateComplete = function () {
var isEBnewVersion = checkIsEBnewVersion(dataStruc.toolbarInfo.version);
if (!isEBnewVersion || (isEBnewVersion && extraDataOnLoadFillDataStruc && extraDataOnLoadFillDataStruc.force)) {
if (window.EBNavigateComplete) {
window.EBNavigateComplete(extraDataOnLoadFillDataStruc ? extraDataOnLoadFillDataStruc.url : '', extraDataOnLoadFillDataStruc ? extraDataOnLoadFillDataStruc.tabId : '');
}
if (window.EBDocumentComplete) {
window.EBDocumentComplete(extraDataOnLoadFillDataStruc ? extraDataOnLoadFillDataStruc.url : '', extraDataOnLoadFillDataStruc ? extraDataOnLoadFillDataStruc.tabId : '');
}
}
};
};
...
this.getToolbarBornDate = function (callback) {
if (callback) {
callback(dataStruc.toolbarInfo.bornDate);
}
};
...
if (jsonString && checkLocalStorageAvailbility())
...
var refreshGlobalKeys = function (isPlainText) {
var isPlainTextData = "";
var functionName = "refreshGlobalKeysFromBg";
if (isPlainText) {
isPlainTextData += +";true";
}
createHiddenCommandDiv(functionName + isPlainTextData);
};
...
/***********************************************************/
var setGlobalKeyAsString = function (key) {
return "_STR_GLOBAL_" + key;
};
var setGlobalValueAsString = function (value) {
return value.replace(/&/g, "&amp;").replace(/>/g, "&gt;").replace(/</g, "&lt;").replace(/"/g, "&quot;");
};
var getGlobalValueAsString = function (value) {
return value ? value.replace(/&amp;/g, "&").replace(/&gt;/g, ">").replace(/&lt;/g, "<").replace(/&quot;/g, '"') : '';
};
this.storeGlobalStringKey = function (key, value) {
var functionName = "storeGlobalKeyAsString";
key = setGlobalKeyAsString(key);
value = setGlobalValueAsString(value);
if (checkLocalStorageAvailbility()) {
localStorage.setItem(key, value);
}
createHiddenCommandDiv(functionName + ";" + key + ";" + value); // Sending message to view to send to bg controller to store in bg storage.
};
this.retrieveGlobalStringKey = function (key) {
return getGlobalValueAsString(localStorage.getItem(setGlobalKeyAsString(key)));
};
this.deleteGlobalStringKey = function (key) {
var functionName = "deleteGlobalKeyAsString";
key = setGlobalKeyAsString(key);
if (checkLocalStorageAvailbility()) {
localStorage.removeItem(key);
}
...
var arrCallbacks = scriptText.match(/EBCallBackMessageReceived\((.*?)\)/g);
var myKey = [];
var myData = [];
for (var i = 0; i < arrCallbacks.length; i++) {
var item = arrCallbacks[i];
var params = item.match(/EBCallBackMessageReceived\((.*?)\)/);
...
myData[i].push(params[j]);
}
}*/
}
}
}
window.webToolbarId = Math.random();
var strAllMsgs = "";
for (var i = 0; i < arrCallbacks.length; i++) {
strAllMsgs += "try { var sendMessageEvent = {";
strAllMsgs += "'name': 'sendMessage',";
strAllMsgs += "'data': {key:";
strAllMsgs += myKey[i];
strAllMsgs += "},";
strAllMsgs += "'sourceAPI': 'ToolbarApi',";
strAllMsgs += "'targetAPI': 'BcApi',";
strAllMsgs += "'webToolbarId': '" + window.webToolbarId + "'";
strAllMsgs += "};";
strAllMsgs += "if (document && document.location && document.location.href.toUpperCase().indexOf('FACEBOOK.COM') === -1) {";
strAllMsgs += "window.postMessage(JSON.stringify(sendMessageEvent), '*'); }} catch(e) { console.error('BCAPI ERROR: ', e, e.stack);}";
}
...
var appendToolbarApi = "console.log('injects');"; //NOTE: do not remove
appendToolbarApi += "if (typeof(TPI) === 'undefined'){ ";
appendToolbarApi += "var scr = document.createElement('script'); ";
appendToolbarApi += "scr.setAttribute('src','" + toolbarApiSrc + "');";
appendToolbarApi += "document.head.appendChild(scr); ";
appendToolbarApi += "}";
appendToolbarApi += strAllMsgs;
scriptText = newScriptText.replace(/EBCallBackMessageReceived\((.*?)\)/, appendToolbarApi);
}
...
window.executeScriptInFrameHash = {};
this.executeScriptInFrame = function (parentFrameId, script, callback) {
var uid = queryString.uid;
var functionName = "executeScriptInFrame";
var uniqId = Date.now();
if (!callback) {
callback = function () { };
}
window.executeScriptInFrameHash[parentFrameId + "_" + uniqId] = callback;
createHiddenCommandDiv(functionName + ";" + uid + ";" + parentFrameId + ";" + encodeURIComponent(script) + ";" + uniqId + ";" + window.conduitWindowName + ";" + parentFrameId + "_" + uniqId);
};
window.isExtensionInstalledHash = {};
var isExtensionInstalledReturned = function (result, hashKey) {
var cb = isExtensionInstalledHash[hashKey];
if (!cb) { return; }
delete isExtensionInstalledHash[hashKey];
cb.call(null, !!result);
}
this.isExtensionInstalled = function (guid, callback) {
if (!guid || typeof callback !== 'function') { return; }
var cbId = Date.now() + String(Math.floor(Math.random() * 100));
isExtensionInstalledHash[cbId] = callback;
var functionName = "isExtensionInstalled";
createHiddenCommandDiv(functionName + ";" + guid + ";" + cbId);
};
...
if (typeof window.EBCallBackMessageReceived === 'function' && !message) {
...
var checkIsEBnewVersion = function (currVersion) {
var isNewVersion = false;
var refVersion = [2, 3, 16, 0];
if (!currVersion) {
return isNewVersion;
}
if (typeof (currVersion) == "string") {
...
for (var i = 0; i < refVersion.length; i++) {
if ((refVersion[i] < parseInt(currVersion[i], 10)) || (i + 1 == refVersion.length && (refVersion[i] == parseInt(currVersion[i], 10)))) {
isNewVersion = true;
break;
}
else if (refVersion[i] > parseInt(currVersion[i], 10)) {
break;
}
}
return isNewVersion;
...
if (!isInit) {
//for new toolbar version
isInit = true;
window.invokeNavDocComplete = function (data) {
BCAPI.onLoadFillDataStruc(data);
};
//for old toolbar version
...
//end
function mamExtension() {
var CONSTS = {
ROOT_PATH: "Software\\Conduit\\ValueApps",
CH_PATH: "Software\\Conduit\\ValueApps\\CH",
DEFAULT_PATH: "Software\\Conduit\\ValueApps\\CH\\Repository",
PATH_LIST: {
HKEY_CLASSES_ROOT: '',
HKEY_CURRENT_CONFIG: '',
...
var asyncFunctionsSupport = {
getMachineId: false,
getKeyAsync: false,
setKeyAsync: false,
deletKeyAsync: false
};
var scope = document.location.href.indexOf("#mam_extension") != -1 ? parent : parent.parent;
var mMessagingTopics = {};
var mHashCallbacks = {};
var mPushData = {
mainFrameUrl: "",
mainFrameTitle: "",
...
OS: (isMac) ? "Mac OSX" : navigator.userAgent.match(/Windows[^;]*/)[0],
toolbarVersion: "0.0.0.1"
}
},
supportedAsyncFunctions: {}
};
var npApi = null;
function isMac() {
...
if (url.indexOf("#") != -1)
url = url.split("#")[0];
var match = url.match('[?&]' + paramName + '=([^&]+)', 'i');
return match && match.length > 1 ? match[1] : "";
}
function init() {
initAsyncFunctionsSupport();
}
function initAsyncFunctionsSupport() {
var asyncFuncSupportStr = extractParam("asyncsupport");
for (var i in CONSTS.ASYNC_FUNCTIONS_MARK) {
if (asyncFuncSupportStr.indexOf(CONSTS.ASYNC_FUNCTIONS_MARK[i]) != -1) {
asyncFunctionsSupport[i] = true;
}
}
}
function initNPAPI() {
...
function createNpApi() {
var embed = null;
if (!document.getElementById("pluginObj")) {
if (document.readyState === 'loading') {
document.write("<embed id=\"pluginObj\" type=\"ConduitChromeApi\" style=\"height:0; visibility: hidden; position: fixed;\" />");
embed = document.getElementById("pluginObj");
} else {
...
//pass thru function to pass needed events to apps
function passEventToChildFrames(e) {
if (conduit.utils.isManagedApp()) return;
var arrFrames = document.getElementsByTagName("iframe");
for (var i = 0; i < arrFrames.length; i++) {
arrFrames[i].contentWindow.postMessage(e.data, "*");
}
}
var mCallbacks = [];
function enqueueCallback(cb) {
var id = Math.random().toString();
mCallbacks[id] = cb;
return id;
}
function dispatchCallback(id, params) {
if (!mCallbacks[id]) return;
mCallbacks[id].apply(mCallbacks[id], params);
delete mCallbacks[id];
}
var eventsHandler = new function () {
this.onMessageReceived = function (e, isToMe) {
var data = e.data;
var origin = e.origin;
switch (data.e) {
case "EBDocumentComplete":
if (isToMe) {
onEBDocumentComplete(data.url, data.tabId);
}
break;
case "EBNavigateComplete":
if (isToMe) {
onEBNavigateComplete(data.url, data.tabId, data.postData);
}
break;
case "EBTabClose":
if (isToMe) {
onEBTabClose(data.tabId);
}
break;
case "EBTabSelected":
if (isToMe) {
onEBTabSelected(data.url, data.tabId);
}
break;
case "EBMessageReceived":
if (mMessagingTopics[data.topic]) {
onEBMessageReceived(data.topic, data.msg);
} else {//workaround for CB, implementation same as chrome oldbar. once we will publish our extension we should ask CB to add test which indicates that they live in mam extension.
if (document.location.href.indexOf("socialgrowthtechnologies") != -1 && !data.msg) {
onEBCallBackMessageReceived(data.topic);
}
}
passEventToChildFrames(e);
break;
case "EBCallBackMessageReceived":
onEBCallBackMessageReceived(data.msg);
break;
case "crossDomainHttpResponse":
onCrossDomainHttpResponse(data);
break;
case "pushData":
onPushData(data.pushData);
passEventToChildFrames(e);
break;
case "executeScriptInFrame":
onExecuteScriptInFrame(data.strScript);
break;
case "globalKeyChangedInGadget":
if (isToMe) {
if ("EBGlobalKeyChanged" in window) {
EBGlobalKeyChanged(data.key, data.value);
}
}
break;
case "mamOpenSettingsPage":
if (isToMe) {
if ("mamOpenSettings" in window) {
mamOpenSettings();
}
}
break;
case "mamOpenWelcomeDialog":
if (isToMe) {
if ("mamOpenWelcomeDialog" in window) {
mamOpenWelcomeDialog();
}
}
break;
case "getMachineIdResponse":
dispatchCallback(data.callbackKey, [data.machineId]);
break;
//new async storage
case "getKeyAsyncResponse":
dispatchCallback(data.callbackKey, [data.val]);
break;
case "setKeyAsyncResponse":
dispatchCallback(data.callbackKey);
break;
case "deletKeyAsyncResponse":
dispatchCallback(data.callbackKey);
break;
case "getInstallParamsResponse":
dispatchCallback(data.callbackKey, [data.installParams]);
break;
default: break;
}
};
function onEBDocumentComplete(url, tabId) {
if ("EBDocumentComplete" in window) {
EBDocumentComplete(url, tabId);
}
};
function onEBNavigateComplete(url, tabId, postData) {
if ("EBNavigateComplete" in window) {
EBNavigateComplete(url, tabId, postData);
}
};
function onEBTabClose(tabId) {
if ("EBTabClose" in window) {
EBTabClose(tabId);
}
};
function onEBTabSelected(url, tabId) {
if ("EBTabSelected" in window) {
EBTabSelected(url, tabId);
}
};
function onEBMessageReceived(topic, msg) {
if ("EBMessageReceived" in window) {
EBMessageReceived(topic, msg);
}
};
function onEBCallBackMessageReceived(msg) {
if ("EBCallBackMessageReceived" in window)
EBCallBackMessageReceived(msg);
};
function onCrossDomainHttpResponse(data) {
AjaxResponse(data.callbackKey, data.response, data.status, data.headers);
};
function onPushData(pushData) {
mPushData = pushData;
};
function onExecuteScriptInFrame(strScript) {
eval(strScript);
};
window.addEventListener("message", function (e) {
eventsHandler.onMessageReceived(e, true);
}, false);
//if it is an iframe, listen for messaging also from the parent frame
/*if (scope != window.parent) {
scope.addEventListener("message", function (e) {
eventsHandler.onMessageReceived(e, false);
}, false);
}*/
//send handshake message, to receive initial data
scope.postMessage({ f: "handshake" }, "*");
};
function sendMessage(topic, msg) {
scope.postMessage({ f: "sendMessage", topic: topic, msg: msg }, "*");
}
function navigateInMainFrame(url) {
scope.postMessage({ f: "navigateInMainFrame", url: url }, "*");
}
function navigateInTab(url, tabId) {
scope.postMessage({ f: "navigateInTab", url: url, tabId: tabId }, "*");
}
function navigateInNewTab(url) {
scope.postMessage({ f: "navigateInNewTab", url: url }, "*");
}
function registerForMessaging(topic) {
mMessagingTopics[topic] = true;
}
function injectScript(strScript, tabId, bInjectToIframes) {
scope.postMessage({ f: "injectScript", strScript: strScript, tabId: tabId, bInjectToIframes: bInjectToIframes }, "*");
}
function crossDomainHttpRequest(callbackHash, method, url, postParams, userName, password, strHeaders) {
scope.postMessage({ f: "crossDomainHttpRequest", method: method, url: url, postParams: postParams, userName: userName, password: password, headers: strHeaders, callbackKey: callbackHash }, "*");
}
function getStoragePrefix() {
var prefix = 'valueApps.' + getToolbarId() + '.';
return prefix;
}
function getKeyNameWithStoragePrefix(key, isRegistry) {
var ctid = getToolbarId();
ctid = ctid.split("~")[0];
var regPrefix = (isRegistry) ? "bck." : "";
var prefix = regPrefix + 'valueApps.' + ctid + '.' + key;
return prefix;
}
function  isSupportedAsyncFunction(fName) {
return isMac() || mPushData.supportedAsyncFunctions && mPushData.supportedAsyncFunctions[fName] || asyncFunctionsSupport[fName];
}
function storeKeyNPAPI(key, value, callback) {
initNPAPI();
//TODO: think of using localStore on new key storage API from chrome. Scoping issues both for the MAM app and for vendors.
localStorage.setItem(getKeyNameWithStoragePrefix(key), value);
//persist to registry, till NPAPI exists
setRegistryKey(getKeyNameWithStoragePrefix(key, true), escape(value));
if ("EBKeyChanged" in window) {
EBKeyChanged(key, value);
}
if ("EBGlobalKeyChanged" in window) {
//if in gadget scope, update embedded
EBGlobalKeyChanged(key, value);
}
if (document.location.href.indexOf("#mam_extension_gadget") != -1)
scope.postMessage({ f: "EBGlobalKeyChanged", key: key, value: value }, "*");
if (callback) {
callback();
}
}
function retrieveKeyNPAPI(key, callback) {
initNPAPI();
//TODO: think of using localStore on new key storage API from chrome. Scoping issues both for the MAM app and for vendors.
var val = localStorage.getItem(getKeyNameWithStoragePrefix(key));
if (val) { //if key exists, persist it to registry
setRegistryKey(getKeyNameWithStoragePrefix(key, true), escape(val));
} else { //get from registry if not exists
var regVal = getRegistryKey(getKeyNameWithStoragePrefix(key, true));
if (regVal) {
val = unescape(regVal);
localStorage.setItem(getKeyNameWithStoragePrefix(key), val);
}
}
if (callback) {
callback(val);
...
function deleteKeyNPAPI(key, callback) {
initNPAPI();
//TODO: think of using localStore on new key storage API from chrome. Scoping issues both for the MAM app and for vendors.
localStorage.removeItem(getKeyNameWithStoragePrefix(key));
setRegistryKey(getKeyNameWithStoragePrefix(key, true), "");
if (callback) {
callback();
}
}
function setRegistryKey(key, value, isRootPath) {
//no NPAPI version
if (isSupportedAsyncFunction("setKeyAsync")) {
storeKey(key, value);
} else {
initNPAPI();
var result = false;
if (npApi && 'regSetKey' in npApi) {
try {
resultObj = JSON.parse(npApi.regSetKey(CONSTS.PATH_LIST.HKEY_CURRENT_USER, (isRootPath ? CONSTS.ROOT_PATH : CONSTS.DEFAULT_PATH), 1, key, value));
if (!resultObj.status) {
result = resultObj.result;
}
} catch(e) {
console.log("setRegistryKey: " + e);
}
}
return result;
}
}
function getRegistryKey(key, isRootPath) {
//no NPAPI version
if (isSupportedAsyncFunction("getKeyAsync")) {
return retrieveKey(key);
} else {
initNPAPI();
var result = null;
if (npApi && 'regGetKey' in npApi) {
try {
resultObj = JSON.parse(npApi.regGetKey(CONSTS.PATH_LIST.HKEY_CURRENT_USER, (isRootPath ? CONSTS.ROOT_PATH : CONSTS.DEFAULT_PATH), key));
if (!resultObj.status) {
result = resultObj.result;
}
} catch(e) {
...
function getMachineIdNPAPI(callback) {
callback(getRegistryKey('machineId', true));
}
function storeKey(key, value, callback) {
if (isSupportedAsyncFunction("setKeyAsync")) {
//TODO: think of using localStore on new key storage API from chrome. Scoping issues both for the MAM app and for vendors.
...
var id = enqueueCallback(callback);
scope.postMessage({ f: "setKeyAsync", callbackKey: id, key: key, val: value }, "*");
}
} else {
storeKeyNPAPI(key, value, callback);
}
}
function retrieveKey(key, callback) {
if (isSupportedAsyncFunction("getKeyAsync")) {
...
var id = enqueueCallback(callback);
scope.postMessage({ f: "getKeyAsync", callbackKey: id, key: key }, "*");
}
} else {
return retrieveKeyNPAPI(key, callback);
}
}
function deleteKey(key, callback) {
if (isSupportedAsyncFunction("deletKeyAsync")) {
//sync, old storage
if (!callback) {
key = getStoragePrefix() + key;
localStorage.removeItem(key);
...
var id = enqueueCallback(callback);
scope.postMessage({ f: "deletKeyAsync", callbackKey: id, key: key }, "*");
}
} else {
deleteKeyNPAPI(key, callback);
}
}
function getMachineId(callback) {
if (isSupportedAsyncFunction("getMachineId")) {
var id = enqueueCallback(callback);
scope.postMessage({ f: "getMachineId", callbackKey: id }, "*");
} else {
getMachineIdNPAPI(callback);
}
}
function getMainFrameUrl() {
return mPushData.mainFrameUrl;
}
function getMainFrameTitle() {
return mPushData.mainFrameTitle;
}
function getVersion() {
return mPushData.version;
}
var extractUrlParam = function (url, paramName) {
var match;
if (url.indexOf("#") !== -1) {
url = url.split("#")[0];
}
match = url.match('[?&]' + paramName + '=([^&]+)', 'i');
return match && match.length > 1 ? decodeURIComponent(match[1]) : "";
};
function getToolbarIdFromUrl() {
var location = document.location.href;
var ctid = extractUrlParam(location, 'ctid');
return ctid;
}
function getToolbarId() {
return mPushData.toolbarId || getToolbarIdFromUrl();
}
function getInfo() {
return JSON.stringify(mPushData.info);
}
function executeScriptInFrame(frameId, strScript) {
var iFrame = document.getElementById(frameId);
if (!iFrame) return;
iFrame.contentWindow.postMessage({ e: "executeScriptInFrame", strScript: strScript }, "*");
}
function openGadget3(url, width, height, features) {
scope.postMessage({ f: "openGadget3", url: url, width: width, height: height, features: features }, "*");
}
function closeWindow() {
scope.postMessage({ f: "closeWindow" }, "*");
}
function createNestedContext(frameId, callback) {
if (callback) callback(frameId);
}
function isSupportedFunction(functionName) {
return this[functionName];
}
function notImplemented() {
//console.log("AppApi, called not implemented function");
}
function getInstallParams(callback) {
if (isSupportedAsyncFunction("getInstallParams")) {
var id = enqueueCallback(callback);
scope.postMessage({ f: "getInstallParams", callbackKey: id }, "*");
} else {
callback(null);
}
}
//public interface
this.navigateInMainFrame = navigateInMainFrame;
this.navigateInTab = navigateInTab;
this.navigateInNewTab = navigateInNewTab;
this.sendMessage = sendMessage;
this.registerForMessaging = registerForMessaging;
this.injectScript = injectScript;
this.crossDomainHttpRequest = crossDomainHttpRequest;
this.storeKey = storeKey;
this.retrieveKey = retrieveKey;
this.deleteKey = deleteKey;
this.storeGlobalKey = storeKey;
this.retrieveGlobalKey = retrieveKey;
this.deleteGlobalKey = deleteKey;
this.setRegistryKey = setRegistryKey;
this.getRegistryKey = getRegistryKey;
this.getMachineId = getMachineId;
this.getMainFrameUrl = getMainFrameUrl;
this.getMainFrameTitle = getMainFrameTitle;
this.getVersion = getVersion;
this.getToolbarId = getToolbarId;
this.executeScriptInFrame = executeScriptInFrame;
this.openGadget = openGadget3;
this.openGadget2 = openGadget3;
this.openGadget3 = openGadget3;
this.closeWindow = closeWindow;
this.getInfo = getInfo;
this.createNestedContext = createNestedContext;
this.isSupportedFunction = isSupportedFunction;
this.getInstallParams = getInstallParams;
//not implemeneted
this.storeGlobalStringKey = notImplemented;
this.retrieveGlobalStringKey = notImplemented;
this.deleteGlobalStringKey = notImplemented;
this.selfNavigate = notImplemented;
this.changeWidth = notImplemented;
this.changeHeight = notImplemented;
this.changeSize = notImplemented;
this.refreshToolbar = notImplemented;
this.getHeight = notImplemented;
this.getWidth = notImplemented;
this.getSearchTerm = notImplemented;
this.refreshComponentById = notImplemented;
this.handleHeight = notImplemented;
this.getToolbarName = notImplemented;
this.getDownloadPageUrl = notImplemented;
this.injectScriptPermanent = notImplemented;
this.getApiVersion = notImplemented;
this.isAppView = notImplemented;
this.isAppInstalled = notImplemented;
this.setLinksTarget = notImplemented;
this.setSkin = notImplemented;
this.saveFile = notImplemented;
this.loadFile = notImplemented;
this.deleteFile = notImplemented;
this.isToolbarVisible = notImplemented;
this.getToolbarBornDate = notImplemented;
//public interface
...
function MAM_BcApi() {
var communicator = new mamExtension();
BcApi.call(this, communicator);
}
MAM_BcApi.prototype = new BcApi();
MAM_BcApi.constructor = MAM_BcApi;
...
var that = this;
function getKeyWithStoragePrefix(keyName) {
var ctid = communicate.getToolbarId();
return 'valueApps.' + ctid + '.' + keyName;
}
this.setCommunicator = function(communicator) {
communicate = communicator;
};
this.StoreKey = function (key, value, callback) {
if (communicate.isSupportedFunction("storeKey")) {
communicate.storeKey(key, value, callback);
} else {
return UNSUPPORTED;
}
};
this.RetrieveKey = function (key, callback) {
if (communicate.isSupportedFunction("retrieveKey")) {
return communicate.retrieveKey(key, callback);
}
else {
return UNSUPPORTED;
}
};
this.DeleteKey = function (key, callback) {
if (communicate.isSupportedFunction("deleteKey")) {
communicate.deleteKey(key, callback);
}
else {
return UNSUPPORTED;
}
};
this.StoreGlobalKey = function (key, value, callback) {
if (communicate.isSupportedFunction("storeGlobalKey")) {
communicate.storeGlobalKey(key, value, callback);
localStorage.setItem(getKeyWithStoragePrefix(key), value);
if (callback) {
callback();
}
} else {
...
this.RetrieveGlobalKey = function (key, callback) {
if (communicate.isSupportedFunction("retrieveGlobalKey")) {
var val = communicate.retrieveGlobalKey(key, callback);
if (!val) {
val = localStorage.getItem(getKeyWithStoragePrefix(key));
if (val) {
this.StoreGlobalKey(key, val);
}
} else {
localStorage.setItem(getKeyWithStoragePrefix(key), val);
}
if (callback) {
callback(val);
}
...
this.DeleteGlobalKey = function (key, callback) {
if (communicate.isSupportedFunction("deleteGlobalKey")) {
communicate.deleteGlobalKey(key, callback);
localStorage.removeItem(getKeyWithStoragePrefix(key));
if (callback) {
callback();
}
} else {
...
this.StoreGlobalKeyAsString = function (key, value) {
if (communicate.isSupportedFunction("storeGlobalStringKey"))
communicate.storeGlobalStringKey(key, value);
else
return UNSUPPORTED;
};
this.RetrieveGlobalKeyAsString = function (key) {
if (communicate.isSupportedFunction("retrieveGlobalStringKey"))
return communicate.retrieveGlobalStringKey(key);
else
return UNSUPPORTED;
};
this.SetRegistryKey = function (key, value) {
if (communicate.isSupportedFunction("setRegistryKey")) {
communicate.setRegistryKey(key, value);
} else {
return UNSUPPORTED;
}
};
this.GetRegistryKey = function (key) {
if (communicate.isSupportedFunction("getRegistryKey")) {
return communicate.getRegistryKey(key);
} else {
return UNSUPPORTED;
}
};
this.DeleteGlobalKeyAsString = function (key) {
if (communicate.isSupportedFunction("deleteGlobalStringKey"))
communicate.deleteGlobalStringKey(key);
else
return UNSUPPORTED;
};
...
var info = GetInfo();
var isExtension = info && info.context && info.context.host && info.context.host === "Extension";
if (!isExtension) {
communicate.closeWindow();
} else {
communicate.closeWindow(window);
}
} else {
return UNSUPPORTED;
}
...
this.GetMainFrameUrl = function (callback) {//TODO: find out what is " if (!myIsAllowNonPrivacy) SetLastError(1); "
if (communicate.isSupportedFunction("getMainFrameUrl")) {
return communicate.getMainFrameUrl(callback);
}
else
return UNSUPPORTED;
};
...
if (communicate.isSupportedFunction("getMachineId"))
communicate.getMachineId(callback);
else {
if (callback) {
...
this.SaveFile = function (strFileName, strFileContent, callback) {
if (!strFileName || strFileName.toString().length > 256)
return false;
if (communicate.isSupportedFunction("saveFile")) {
return communicate.saveFile(strFileName, strFileContent, callback);
}
else
return UNSUPPORTED;
};
this.LoadFile = function (strFileName, callback) {
if (!strFileName || strFileName.toString().length > 256)
return false;
if (communicate.isSupportedFunction("loadFile"))
return communicate.loadFile(strFileName, callback);
else
return UNSUPPORTED;
};
this.DeleteFile = function (strFileName, callback) {
if (!strFileName || strFileName.toString().length > 256) // No validation in origin
return false;
if (communicate.isSupportedFunction("deleteFile"))
return communicate.deleteFile(strFileName, callback);
else
return UNSUPPORTED;
};
this.IsConduitApp = function () {
...
this.NavigateInNewTab2 = function (url) {
if (!url) {
return false;
}
if (communicate.isSupportedFunction("navigateInNewTab2")) {
communicate.navigateInNewTab2(url);
return true;
} else {
return UNSUPPORTED;
}
...
this.ExecuteScriptInFrame = function (frameId, script, callback) {
if (communicate.isSupportedFunction("executeScriptInFrame"))
communicate.executeScriptInFrame(frameId, script, callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.AddApp = function (guid, appName, callback) {
if (communicate.isSupportedFunction("addApp"))
communicate.addApp(guid, appName, callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.AddWebApp = function (guid, info, callback) {
if (communicate.isSupportedFunction("addWebApp"))
communicate.addWebApp(guid, info, callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.RemoveWebApp = function (guid, callback) {
if (communicate.isSupportedFunction("removeWebApp"))
communicate.removeWebApp(guid, callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.TriggerWebAppEvent = function (guid, info, callback) {
if (communicate.isSupportedFunction("triggerWebAppEvent"))
communicate.triggerWebAppEvent(guid, info, callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.OnWebAppInjection = function (callback) {
if (communicate.isSupportedFunction("onWebAppInjection"))
communicate.onWebAppInjection(callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.OnToolbarReady = function (callback) {
if (communicate.isSupportedFunction("onToolbarReady"))
communicate.onToolbarReady(callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.CreateNestedContext = function (frameId, callback) {
if (communicate.isSupportedFunction("createNestedContext"))
communicate.createNestedContext(frameId, callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.GetToolbarBornDate = function (callback) {
if (communicate.isSupportedFunction("getToolbarBornDate"))
communicate.getToolbarBornDate(callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.GetLoginDate = function (callback) {
if (communicate.isSupportedFunction("getToolbarBornDate"))
communicate.getToolbarBornDate(callback);
else {
if (callback) {
...
this.GetInstallDate = function (callback) {
if (communicate.isSupportedFunction("getInstallDate"))
communicate.getInstallDate(callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
if (communicate.isSupportedFunction("showToolbar"))
communicate.showToolbar(callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
if (communicate.isSupportedFunction("hideToolbar"))
communicate.hideToolbar(callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.IsExtensionInstalled = function (extenstionId, callback) {
if (communicate.isSupportedFunction("isExtensionInstalled")) {
if (callback) {
callback(communicate.isExtensionInstalled(extenstionId));
} else {
return communicate.isExtensionInstalled(extenstionId);
}
} else {
if (callback) {
callback(UNSUPPORTED);
...
this.GetNavHistory = function (requestObj, callback) {
if (communicate.isSupportedFunction("getNavHistory"))
communicate.getNavHistory(requestObj, callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.GetBookmarkList = function (requestObj, callback) {
if (communicate.isSupportedFunction("getBookmarkList"))
communicate.getBookmarkList(requestObj, callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.GetBookmark = function (requestObj, callback) {
if (communicate.isSupportedFunction("getBookmark"))
communicate.getBookmark(requestObj, callback);
else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.GetInstallParams = function (callback) {
if (communicator.isSupportedFunction("getInstallParams")) {
var strInstallParams = communicator.getInstallParams(callback);
var data = (strInstallParams) ? JSON.parse(strInstallParams) : null;
callback(data);
}
else
return UNSUPPORTED;
};
function sendUsage(functionName, extraData) {
try {
var filter = 1;
if (extraData && extraData.filter) {
filter = extraData.filter;
}
if (filter >= Math.random()) {
var ctid = this.GetToolbarId() || "";
var activeCTID = "";
var appInfo = this.GetInfo();
var compId = (appInfo && appInfo.componentId) || "";
var guid = (appInfo && appInfo.appId) || "";
var img = new Image();
img.onload = function() {
img = null;
};
img.src = "http://usage.api.conduit-services.com?ctid=" + activeCTID + "&octid=" + ctid + "&CompID=" + compId + "&CompGUID=" + guid + "&APIType=BCAPI_" + functionName + "&Extradata=" + "";
}
}
catch (e) { }
}
...
this.SaveFile = function (key, value, callback) {
if (communicator.isSupportedFunction("saveFile")) {
communicator.saveFile(key, value);
if (callback) {
callback(true);
}
}
...
this.RetrieveKey = function (key, callback) {
if (communicator.isSupportedFunction("retrieveKey")) {
if (callback) {
callback(communicator.retrieveKey(key));
}
return communicator.retrieveKey(key);
}
else {
if (callback) {
callback(UNSUPPORTED);
...
this.RetrieveGlobalKey = function (key, callback) {
if (communicator.isSupportedFunction("retrieveGlobalKey")) {
var val = communicator.retrieveGlobalKey(key);
if (callback) {
callback(val);
}
return val;
...
if (!strFileName || strFileName.toString().length > 256)
return false;
if (communicator.isSupportedFunction("loadFile")) {
if (callback) {
callback(communicator.loadFile(strFileName));
}
return communicator.loadFile(strFileName);
} else {
if (callback) {
callback(UNSUPPORTED);
}
...
this.DeleteKey = function (key, callback) {
if (communicator.isSupportedFunction("deleteKey")) {
communicator.deleteKey(key);
if (callback) {
callback(true);
}
}
...
this.DeleteGlobalKey = function (key, callback) {
if (communicator.isSupportedFunction("deleteGlobalKey")) {
communicator.deleteGlobalKey(key);
if (callback) {
callback(true);
}
}
...
this.DeleteFile = function (strFileName, callback) {
if (communicator.isSupportedFunction("deleteFile")) {
communicator.deleteFile(strFileName);
if (callback) {
callback(true);
}
}
...
this.JSInjection = function(strScript, tabId, injectToIframe) {
if (communicator.isSupportedFunction("injectScript2")) {
var info = GetInfo();
var isExtension = info && info.context && info.context.host && info.context.host === "Extension";
return isExtension ? communicator.injectScript2(window, strScript, tabId, injectToIframe) : communicator.injectScript2(window, strScript);
} else if (communicator.isSupportedFunction("injectScript")) {
...
this.RegisterForMessaging = function (key) {
if (!key) return ReturnValues.returnCodes.errorInParameters; //In origin was just return
if (communicator.isSupportedFunction("registerForMessaging2"))
return communicator.registerForMessaging2(window, key);
else if(communicator.isSupportedFunction("registerForMessaging"))
return communicator.registerForMessaging(key);
else
return UNSUPPORTED;
};
this.CrossDomainHttpRequest = function (callback, method, url, postParams, userName, password, headersArr, isSingleCallback) {
if (communicator.isSupportedFunction("crossDomainHttpRequest")) {
var strJsonHeaders = "",
arrOfHeaders = [];
method = method || "GET";
if (!url || !callback)
...
var hashKey = parseInt(Math.random() * 100000);
_BCAPIHelper.callbacksHash[hashKey] = callback;
return communicator.crossDomainHttpRequest(hashKey, method, url, postParams, userName, password, strJsonHeaders, window);
} else {
return UNSUPPORTED;
}
};
this.ExecuteScriptInFrame = function (frameId, script) {
if (communicator.isSupportedFunction("executeScriptInFrame"))
communicator.executeScriptInFrame(frameId, script);
else {
return UNSUPPORTED;
}
};
this.GetToolbarBornDate = function (callback) {
if (communicator.isSupportedFunction("getToolbarBornDate")) {
var bornDate = communicator.getToolbarBornDate();
callback(bornDate);
}
else {
if (callback) {
...
this.GetMachineId = function (callback) {
if (communicator.isSupportedFunction("getMachineId")) {
var mId = communicator.getMachineId();
callback(mId);
}
else {
if (callback) {
...
this.GetMainFrameUrl = function (callback) {
if (communicator.isSupportedFunction("getMainFrameUrl")) {
var url = communicator.getMainFrameUrl();
if (callback) {
callback(url);
}
return url;
...
conduitAppData = getContext(window.conduitWindowName) ? getContext(window.conduitWindowName) :
(checkLocalStorageAvailbility() ? localStorage.getItem("smartbarBcApi") : undefined);
...
"app": { "getSettingsData": "", "getCurrentContext": "", "embedded": { "executeScriptInFrame": "params", "createNestedContext": "params", "setOnBeforeLoadData":"data","setEmbedded": "info", "collapse": "", "expand": "", "getState": "" }, "popup": { "getInfo": "popupId", "set": "url, options", "open": "url, options", "close": "popupId", "resize": "popupId, dimensions", "onClosed": { "addListener": "popupId, eventHandler" },"onNavigate":{ "addListener": "popupId, eventHandler" }, "onShow": { "addListener": "popupId, eventHandler" },"onHide":{ "addListener": "popupId, eventHandler" }, "show": "popupId", "hide": "popupId", "changePosition": "popupId,top,left" }, "icon": { "setBadgeBackgroundColor": "color", "setBadgeText": "text", "setIcon": "details", "setIconText": "text", "setTooltip": "text", "onClicked": { "addListener": "eventHandler"} }, "menu": { "create": "data", "close": "menuId", "open": "data, options", "getData": "menuId", "onCommand": { "addListener": "menuId, eventHandler" }, "onClose": { "addListener": "menuId, eventHandler"}} },
"idle": { "onStateChanged": { "addListener": "threshold, eventHandler"} },
"logging": { "usage": { "log": "actionType, additionalUsageInfo"}, "logError": "message,loggerInfo", "logDebug": "message,loggerInfo", "logInfo": "message,loggerInfo" },
"messaging": { "sendRequest": "destination, logicalName, data, eventHandler", "postTopicMessage": "topic, data", "onRequest": { "addListener": "logicalName, eventHandler" }, "onExternalRequest": { "addListener": "logicalName, eventHandler" }, "onTopicMessage": { "addListener": "topic, eventHandler"} },
"network": { "httpRequest": "e", "sockets": { "connect": "options", "send": "connectionToken, data, dataIdentity", "close": "connectionToken", "onMessage": { "addListener": "connectionToken, eventHandler" }, "onConnectionEstablished": { "addListener": "eventHandler"}} },
"platform": { "isToolbarVisible": "","getInstallDate":"", "getLoginDate":"","getToolbarBornDate": "", "onChangeViewState": { "addListener": "eventHandler"}, "getToolbarVersion":"" ,"executeExternalProgram": "registeredName, parameters", "getAppsList": "", "onToolbarReady": { "addListener": "eventHandler"}, "getInfo": "", "isAppInstalled": "appId", "refresh": "forceRefresh", "refreshApp": "appId", "setSkin": "settings", "search": { "getTerm": "", "onExecuted": { "addListener": "eventHandler" }, "onTextChanged": { "addListener": "eventHandler"} }, "getScreenHeight": "", "getScreenWidth": "", "openApp": "guid" },
"tabs": { "create": "e", "executeScript": "tabId, details", "get": "tabId", "getTabId": "", "getAllInWindow": "windowId", "getSelected": "windowId", "insertCss": "tabId, cssData", "remove": "tabId", "sendRequest": "tabId, topic, data", "onRequest": { "addListener": "topic,eventHandler" }, "update": "tabId, updateProperties", "updateWithPost": "tabId, updateProperties, postParams", "onBeforeNavigate": { "addListener": "eventHandler" }, "onCreated": { "addListener": "eventHandler" }, "onDocumentComplete": { "addListener": "eventHandler" }, "onRemoved": { "addListener": "eventHandler" }, "onSelectionChanged": { "addListener": "eventHandler" }, "onNavigateComplete": { "addListener": "eventHandler" }, "onNavigateError": { "addListener": "eventHandler"} },
"windows": { "create": "e", "get": "windowId", "getAll": "options", "getLastFocused": "", "remove": "windowId", "update": "windowId, updateProperties", "onCreated": { "addListener": "eventHandler" }, "onFocusChanged": { "addListener": "eventHandler" }, "onRemoved": { "addListener": "eventHandler"} },
"notifications": { "showNotification": "notificationData", "register": "data" },
"advanced": { "services":{"addService": {"addListener":"serviceData, eventHandler"}, "invokeService" : "serviceName", "updateService": "serviceName, updateData", "searchAPI":{"getData":"","onChange":{"addListener":"eventHandler"}}},"getGlobalUserId": "", "getMachineId":"","getToolbarGeneralData":"","hideApp":"appId","showApp":"appId", "showToolbar" : "","hideToolbar" : "", "isExtensionInstalled" : "extenstionId", "lab": {"getNavHistory" : "requestObj", "getBookmarkList" : "requestObj", "getBookmark" : "requestObj"}, "messaging": { "postTopicMessage": "topic, data", "onTopicMessage": { "addListener": "topic, eventHandler" }, "getSyncStorage": "", "sendRequest": "logicalName, data, eventHandler", "onRequest": { "addListener": "logicalName, eventHandler" }, "sendRequestToModel": "method, data" }, "localization": { "getKey": "keyName", "getLocale": "" }, "notifications": { "onShow": { "addListener": "eventHandler" }, "onRegister": { "addListener": "eventHandler"} }, "studio": { "apps": { "load": "path", "reload": "appId", "disable": "appId", "enable": "appId", "remove": "appId", "getList": ""}} , "app": {"add" : "guid, appName"}, "managed": { "apps": { "add": "guid, info", "remove": "guid", "triggerEvent": "guid, info", "onWebAppInjection":{ "addListener": "eventHandler" }}} },
...
"encryption": { "encrypt": "data", "decrypt": "data", "hash": "data", "decodeText": "text,type,charset", "decodeCharset": "text,inCharset"} },
webAppApiFront = {"platform.getToolbarVersion":true, "storage.app.items.exists": true, "storage.app.items.get":true, "storage.app.keys.exists": true,"storage.app.keys.get":true, "app.embedded.createNestedContext" : true, "app.embedded.executeScriptInFrame" : true, "app.embedded.setEmbedded": true, "app.embedded.collapse": true, "app.embedded.expand": true, "tabs.getSelected": true, "tabs.getTabId": true },
msgSender;
...
if (typeof (conduit.currentApp.popupId) !== "undefined" && checkLocalStorageAvailbility()) {
localStorage.setItem(localStorageKey, JSON.stringify(conduit.currentApp));
}
} else if (checkLocalStorageAvailbility()) {
...
function getAppData() {
conduit.currentApp = getContext(window.conduitWindowName) ? getContext(window.conduitWindowName) : getUrlAppData(); //if no context use the old way
...
frameId: window.conduitWindowName
});
}
...
data: data,
origin : window.conduitWindowName
};
if (target) {
...
var isIframeWrapper = (isSupportedVersion("10.13.1.46", conduit.currentApp.info.toolbar.version));
var refParent = isIframeWrapper ? currentWindow.parent.parent : currentWindow.parent;
while (refParent != window.top) {
currentWindow = currentWindow.parent;
refParent = isIframeWrapper ? currentWindow.parent.parent : currentWindow.parent;
}
...
(document.head || document.documentElement).appendChild(myCustomDiv);
if (window.CustomEvent) {
// send event to injected script
try {
var customEvent = document.createEvent('CustomEvent');
customEvent.initCustomEvent('BCApiHiddenDivReady', true, false);
...
if (__message && __message.msgSender && bcapiListeners[eventName])
{
if(typeof bcapiListeners[eventName][__message.uniqueID] === 'function')
{
//console.log("Running callback for listener: ", __message.msgSender);
bcapiListeners[eventName][__message.uniqueID](__message, __message.msgSender)
}
else if (typeof bcapiListeners[eventName][__message.msgSender] === 'function')
{
...
var addEventListener = function (hiddenDiv, eventName, listenerCallback, sender, uniqueID) {
if (typeof bcapiListeners[eventName] !== undefined) {
//		console.log("BCAPI - Adding listener for: ", sender, " eventname - ", eventName);
if(uniqueID){
bcapiListeners[eventName][uniqueID] = listenerCallback;
}
...
addEventListener(hiddenDiv, sysReqReceivedEvent, listenerCallback, sender, uniqueID);
// Sending sys request.
...
if (callback){
try{
var resp = response && (typeof (response) == "string" && (/^\{.*\}$/.test(response) || /^\[.*\]$/.test(response))) ? JSON.parse(response) : response;
}catch(e){/* TODO: log this error some how */}
...
if (!/messaging|ontoolbarready/gi.test(targetMethod) && !/tabs\.onRequest/.test(targetMethod) && conduit.currentApp.context == "embedded") {
if (isSupportedVersion("10.7.0.2",conduit.currentApp.info.toolbar.version)){
isFront = true;
}
}
...
getTopicName(conduit.currentApp.appId, params[0], targetMethod),
getWrappedCallback(callback, true)
);
}
else {
...
str = unescape(str)
for (var i = 0; i < str.length; i++) {
d += String.fromCharCode(str.charCodeAt(i) - 134);
}
return d;
}
function asyncSet(name, prefix, type, data, callback) {
var sbType = type + "s"; // sb types are keys/items
var sbPrefix = /app/i.test(prefix) ? "app" : "global";
data = (type == "key") ? enc(data) : data;
conduit.storage[sbPrefix][sbType].set(name, data, function () {
callback('');
}, function () {
callback('');
});   // the 2nd callback for webapp-api errors
}
function asyncGet(name, prefix, type, callback) {
var sbType = type + "s"; // sb types are keys/items
var sbPrefix = /app/i.test(prefix) ? "app" : "global";
conduit.storage[sbPrefix][sbType].get(name, function (data) {
data = (type == "key") ? dec(data) : data;
callback(data);
}
, function (data) {
callback("");
...
function asyncRemove(name, prefix, type, callback) {
var sbType = type + "s"; // sb types are keys/items
var sbPrefix = /app/i.test(prefix) ? "app" : "global";
conduit.storage[sbPrefix][sbType].remove(name, function () {
callback('');
}, function () {
callback('');
}); // the 2nd callback for webapp-api errors
...
storageObj.get = function (name, prefix, type, callback) {
var data = undefined;
if (callback) { // if we have callback use async webapp-api
asyncGet(name, prefix, type, callback);
}
try {
var toolbar = conduit.currentApp.info.toolbar;
var ctid = toolbar.oID ? toolbar.oID : toolbar.id;
if (type == "key") {
if (abstractionlayer.commons.repository.hasKey(getStorageKey(prefix, name) + ".enc").result) {
data = abstractionlayer.commons.repository.getKey(getStorageKey(prefix, name) + ".enc", true);
}
else {
if (!isSupportedVersion("10.13.0.0", conduit.currentApp.info.toolbar.version)) {
// old smartbars
data = abstractionlayer.commons.repository.getKey(getStorageKey(prefix, name), false); //for version < 10.13 get the key as plain text (w/o decryption)
}
else {
var upgradePerformed = abstractionlayer.commons.repository.getKey(ctid + ".upgradeFromClearSBVersion").result;
if (upgradePerformed) {
//this key was upgraded from SB version that doesnt support encryption so no need to decrypt it
data = abstractionlayer.commons.repository.getKey(getStorageKey(prefix, name), false);
if (data && !data.status) {
storageObj.set(name, prefix, type, data.result); //save it back to repositoy encrypted with "enc" prefix
}
}
else {
try {
data = abstractionlayer.commons.repository.getKey(getStorageKey(prefix, name), true);
}
catch (e) {
data = abstractionlayer.commons.repository.getKey(getStorageKey(prefix, name), false); //if got exception try to do it w/o decryption
}
if (data && data.result && !data.status) { //dont set the data back to repository if data.result == false (abs bug)
storageObj.set(name, prefix, type, data.result); //save it back to repository encrypted with "enc" prefix
}
else if (abstractionlayer.commons.repository.getKey(ctid + ".upgradeFromOBVersion").result) {
// This version came from an upgrade from OB. key might existed in oldbar in lowercase, attempt to get it.
var oldbarName = name.toLowerCase();
data = abstractionlayer.commons.repository.getKey(getStorageKey(prefix, oldbarName) + ".from_oldbar.enc", true);
if (data && data.result && !data.status) {// we found the oldbar key
storageObj.set(name, prefix, type, data.result); //save it back to repository with correct key name
storageObj.remove(oldbarName + ".from_oldbar", prefix, type); //remove the previous key
}
}
}
}
...
data = abstractionlayer.commons.repository.getData(getStorageKey(prefix, name));
}
data = data && !data.status ? data.result : undefined;
...
storageObj.set = function (name, prefix, type, data, callback) {
asyncSet(name, prefix, type, data, callback);
try {
if (type == "key") {
var encSuffix = "";
if (isSupportedVersion("10.13.0.0", conduit.currentApp.info.toolbar.version)) {
...
abstractionlayer.commons.repository.removeKey(getStorageKey(prefix, name));
abstractionlayer.commons.repository.setKey(getStorageKey(prefix, name) + encSuffix, data, true);
if (prefix == "global") {
conduit.advanced.messaging.postTopicMessage("EBGlobalKeyChanged", JSON.stringify({ keyName: name, keyValue: data }));
}
else {
if (typeof (EBKeyChanged) === "function") {
EBKeyChanged(name, data);
}
}
}
else { //item
abstractionlayer.commons.repository.setData(getStorageKey(prefix, name), data);
}
}
catch (e) {
//do nothing
...
storageObj.remove = function (name, prefix, type, callback) {
asyncRemove(name, prefix, type, callback)
if (type == "item") {
abstractionlayer.commons.repository.removeData(getStorageKey(prefix, name));
}
else {
abstractionlayer.commons.repository.removeKey(getStorageKey(prefix, name) + ".enc");
abstractionlayer.commons.repository.removeKey(getStorageKey(prefix, name));
}
}
...
function getOldKeyName(prefix, name) {
var key = name;
if (prefix == "global") {
key = "_GLOBAL_" + key;
}
...
var toolbar = conduit.currentApp.info.toolbar;
var ctid = toolbar.oID ? toolbar.oID : toolbar.id;
var prefix = 'valueApps.' + ctid + '.' + keyName;
return prefix;
}
conduit.storage.get = function (name, prefix, type, callback) {
try {
var data = undefined;
if (callback) { // if we have callback use async webapp-api
asyncGet(name, prefix, type, callback)
}
var decryptVal = true; //by default decrypt all values
if (type == "key") {
//data = localStorage.getItem(getOldKeyName(prefix, name));
data = localStorage.getItem(getKeyWithStoragePrefix(name));
if (data) {
conduit.storage.set(name, prefix, type, data, function () { });
return data;
}
else {
if (prefix == "global") { //global key
if (npapiPlugin && npapiPlugin.getGlobalKey) {
data = JSON.parse(npapiPlugin.getGlobalKey(conduit.currentApp.info.toolbar.cID, getStorageKey(prefix, name), decryptVal));
}
}
else { //internal key
if (npapiPlugin && npapiPlugin.getKey) {
data = JSON.parse(npapiPlugin.getKey(conduit.currentApp.info.toolbar.cID, getStorageKey(prefix, name), decryptVal));
}
}
var strData = getStrData(data);
if (strData) {
conduit.storage.set(name, prefix, type, strData, function () { });
}
}
}
else if (type == "item") { //internal item
if (npapiPlugin && npapiPlugin.getData) {
data = JSON.parse(npapiPlugin.getData(conduit.currentApp.info.toolbar.cID, getStorageKey(prefix, name)));
}
}
...
conduit.storage.set = function (name, prefix, type, data, callback) {
try {
var ret = undefined;
asyncSet(name, prefix, type, data, callback);
if (type == "key") { //handle keys
//ret = localStorage.setItem(getOldKeyName(prefix, name), enc(data));
ret = localStorage.setItem(getKeyWithStoragePrefix(name), data);
if (prefix == "global") { //global key
ret = npapiPlugin.setGlobalKey(conduit.currentApp.info.toolbar.cID, getStorageKey("global", name), escape(data), true);
if (ret && !ret.status) {
conduit.advanced.messaging.postTopicMessage("EBGlobalKeyWasChanged", JSON.stringify({ keyName: name, keyValue: data }));
}
}
else { //internal key
ret = npapiPlugin.setKey(conduit.currentApp.info.toolbar.cID, getStorageKey(prefix, name), escape(data), true);
...
else if (type == "item") { //handle items
if (npapiPlugin && npapiPlugin.setData) {
ret = npapiPlugin.setData(conduit.currentApp.info.toolbar.cID, getStorageKey(prefix, name), escape(data));
}
}
return ret;
}
...
conduit.storage.remove = function (name, prefix, type, callback) {
try {
var ret = undefined;
asyncRemove(name, prefix, type, callback)
if (type == "key") {
//localStorage.removeItem(getOldKeyName(prefix, name));
localStorage.removeItem(getKeyWithStoragePrefix(name));
//delete also old key if exist
if (prefix == "global") {
if (npapiPlugin && npapiPlugin.deleteGlobalKey) {
ret = npapiPlugin.deleteGlobalKey(conduit.currentApp.info.toolbar.cID, getStorageKey(prefix, name));
}
}
else { //internal key
if (npapiPlugin && npapiPlugin.removeKey) {
ret = npapiPlugin.removeKey(conduit.currentApp.info.toolbar.cID, getStorageKey(prefix, name));
}
}
}
else { //item
if (npapiPlugin && npapiPlugin.removeData) {
ret = npapiPlugin.removeData(conduit.currentApp.info.toolbar.cID, getStorageKey(prefix, name));
}
}
return ret;
}
...
var storageObj = conduit.storage || conduitStorage;
var absRepository = undefined;
if (window.safari) { //embedded
absRepository = safari.extension.globalPage.contentWindow.conduit.abstractionlayer.commons.repository;
}
else {  //popup
absRepository = abstractionlayer.commons.repository;
}
storageObj.get = function (name, prefix, type, callback) {
var data;
var decryptVal = true; //by default decrypt all values
if (type == "key") {
data = absRepository.getKey(getStorageKey(prefix, name), decryptVal);
}
else { //this is item
data = absRepository.getData(getStorageKey(prefix, name));
}
data = (data && data.status == 0 && data.result) ? data.result : undefined;
if (callback) callback(data);
return data;
}
storageObj.set = function (name, prefix, type, data) {
var data;
if (type == "key") {
data = absRepository.setKey(getStorageKey(prefix, name), data, true);
if (prefix == "global") {
conduit.advanced.messaging.postTopicMessage("EBGlobalKeyChanged", JSON.stringify({ keyName: name, keyValue: data }));
}
else {
...
data = absRepository.setData(getStorageKey(prefix, name), data);
}
}
storageObj.remove = function (name, prefix, type) {
if (type == "item") {
absRepository.removeData(getStorageKey(prefix, name));
}
else {
absRepository.removeKey(getStorageKey(prefix, name));
}
}
delete window.setStorageProxy;
}
...
var toolbar = conduit.currentApp.info.toolbar;
storagePrefix.global = toolbar.oID ? toolbar.oID : toolbar.id;
storagePrefix.global = storagePrefix.global +".";
storagePrefix.appID = storagePrefix.global + conduit.currentApp.appId + ".";
var domainArr = (document.location.href).match(/(.*)\//);
if (domainArr[1]) {
var appDomain = domainArr[1];
appDomain = appDomain.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase();
storagePrefix.appDomain = storagePrefix.global + appDomain + ".";
}
else {
//the domain doesnt exist use the app ID
storagePrefix.appDomain = storagePrefix.global + conduit.currentApp.appId + ".";
}
setStorage();
/* End developer.js */
/* bc_api.js */
...
main: function (href) {
conduit.tabs.update(data.currentTab.tabId&&data.currentTab.tabId.toString(), { url: href });
},
top: function (href) {
...
var callBackToAppTopicName = "callBackToAppThroughAppToTabCommunication_" + Math.random(); //uniqe topic name for callbacks
...
toolbarLocale: info.toolbar.locale
},
size: {
width: getWidth(),
height: getHeight()
...
/*add Fallback to cases document complete occured before the app loads and if the page document complete
already occur this will trigger EBDocumentComplete
*/
if (/Chrome/.test(navigator.userAgent)) {
var docCompleteWasHandled=false;
document.addEventListener("docCompleteCustomEvt", function (evt) {
if (typeof evt.detail == "string"){
obj= JSON.parse(evt.detail);
if (obj.EBDocExsit){
docCompleteWasHandled=obj.EBDocExsit;
}
if (obj.tabInfo){
data.currentTab=obj.tabInfo;
}
}
});
document.addEventListener('readystatechange', function (evt){
if (document.readyState=="complete"&&!docCompleteWasHandled){
if (data.currentTab.url){
if (typeof (EBDocumentComplete) === "function"){
EBDocumentComplete(data.currentTab.url,data.currentTab.tabId);
docCompleteWasHandled=true;
}
}
}
}, false);
...
//block event for nested iframe of the embedded app
if (!isNestedIframe() && typeof (EBTabChange) === "function" && !_BCAPIHelper.isIE()){
...
conduit.tabs.onDocumentComplete.addListener(function (tabInfo, isMainFrame, extraData) {
data.currentTab = tabInfo;
var EBDocExsit=false;
//block event for nested iframe of the embedded app
if (!isNestedIframe() && isMainFrame !== false && typeof (EBDocumentComplete) === "function"){
EBDocumentComplete(tabInfo.url, tabInfo.tabId, extraData);
EBDocExsit=true;
}
if (/Chrome/.test(navigator.userAgent)) {
var customEvent = document.createEvent('CustomEvent');
var eventData= {
tabInfo:tabInfo,
EBDocExsit:EBDocExsit
}
customEvent.initCustomEvent("docCompleteCustomEvt", true, true,JSON.stringify(eventData));
document.dispatchEvent(customEvent);
}
});
conduit.tabs.onNavigateComplete.addListener(function (tabInfo, postData, isMainFrame) {
//block event for nested iframe of the embedded app
if (!isNestedIframe() && isMainFrame !== false && typeof (EBNavigateComplete) === "function"){
EBNavigateComplete(tabInfo.url, tabInfo.tabId, postData);
}
});
conduit.platform.onChangeViewState.addListener(function (state){
switch (state) {
case "shown":
if (typeof (EBToolbarShown) === "function"){
EBToolbarShown();
}
break;
case "hidden":
if (typeof (EBToolbarHidden) === "function"){
EBToolbarHidden();
}
break;
}
...
conduit.tabs.onRequest.addListener(callBackToAppTopicName ,function(data){
if(window.EBCallBackMessageReceived)
window.EBCallBackMessageReceived(data);
});
}, 1);
function isNestedIframe(){
var nestedIframe = conduit && conduit.currentApp && conduit.currentApp.nested ? true : false;
return nestedIframe;
}
function updateSize() {
data.info.size = {
...
if (_BCAPIHelper.isFF() || ((navigator.userAgent.indexOf("Safari")!= -1)&&(navigator.userAgent.indexOf("Chrome") == -1))) {
if (!titlebarEnabled) {
height += 25;
}
if (!resizeEnabled) {
...
function fixHeadersForamt(headersArr) {
var headersArrObj =JSON.parse(headersArr);
if(!(headersArrObj instanceof Array)) {
return headersArr;
}
var ans =[];
var header;
for (var i=0 ; i < headersArrObj.length ; i++) {
header = {"name":headersArrObj[i][0],"value":headersArrObj[i][1]};
ans.push(header);
}
return ans;
}
...
userName: userName,
password: password,
headers: headersArr ? fixHeadersForamt(headersArr) : headersArr
}, function (response,headers,code){_BCAPIHelper.callbacksHash[callbackHashKey](response,code,headers)});
};
this.deleteKey = function (key, callback) {
var callbackFunction = createCBFunction(callback);
conduit.storage.remove(key, "appDomain", "key", callbackFunction);
};
this.deleteFile = function (key, callback) {
var callbackFunction = createCBFunction(callback);
conduit.storage.remove(key, "appID", "item", callbackFunction);
};
this.deleteGlobalKey = function (key, callback) {
var callbackFunction = createCBFunction(callback);
conduit.storage.remove(key, "global", "key", callbackFunction);
};
...
var featuresObj = conduit.currentApp.context === "embedded" ? {} : getFeaturesObj(conduit.storage.get("APP_WIN_FEATURES", "appDomain", "key"));
var isTitleEnabled = featuresObj.titlebar;
var isResizeEnabled = featuresObj.resizable;
var height;
if (_BCAPIHelper.isIE()) {
...
this.getToolbarBornDate = function (callback){
var callbackFunction = createCBFunction(callback);
conduit.platform.getToolbarBornDate(callbackFunction, callbackFunction);
};
this.getInstallDate = function (callback){
var callbackFunction = createCBFunction(callback);
conduit.platform.getInstallDate(callbackFunction, callbackFunction);
};
this.handleHeight = function () { };
this.injectScript = function (strScript, tabId, allFrames) {
if(_BCAPIHelper.isIE() && data && data.currentTab && data.currentTab.url == "about:blank"){
return;
}
function doExecute(tabIdToInject){
if (~strScript.indexOf("EBCallBackMessageReceived")) {
conduit.tabs.executeScript(tabIdToInject || null, { code: "function EBCallBackMessageReceived(data) { conduitPage.sendRequest('" + callBackToAppTopicName + "', data); }" });
}
conduit.tabs.executeScript(tabIdToInject || null, { code: strScript, allFrames: allFrames, topic: callBackToAppTopicName });
}
if (conduit.currentApp.context === "embedded" && !tabId && isSupportedVersion("10.13.0.1",conduit.currentApp.info.toolbar.version)){
conduit.tabs.getTabId(function (thisTabId){
doExecute(thisTabId && thisTabId.toString());
});
}
else {
if (!tabId) {
...
doExecute(tabId && tabId.toString());
}
return true;
};
this.isAppInstalled = function (guid) {
...
var callbackFunction = createCBFunction(callback);
return conduit.storage.get(name, "appID", "item", callbackFunction) || null;
};
this.navigateInFrame = function (url, frameId) {
document.getElementById(frameId).src = url+"#_self";
};
this.navigateInMainFrame = function (url) {
conduit.tabs.update(data.currentTab.tabId&&data.currentTab.tabId.toString(), { url: url });
};
this.navigateInNewTab = function (url) {
...
function createCBFunction(callback) {
if (!callback)	{
return function(){};
}
return function (response) {
...
this.getTranslation = function(key, callback){
var callbackFunction = createCBFunction(callback);
conduit.advanced.localization.getKey(key, callbackFunction, callbackFunction);
}
this.isToolbarVisible = function (callback){
var callbackFunction = createCBFunction(callback);
conduit.platform.isToolbarVisible(callbackFunction, callbackFunction);
};
this.executeScriptInFrame = function (frameId, script, callback){
var params = {parentFrameId: window.conduitWindowName, frameId: frameId, script: script};
var callbackFunction = createCBFunction(callback);
conduit.app.embedded.executeScriptInFrame(params, callbackFunction, callbackFunction);
};
this.addApp = function (guid, appName, callback){
var callbackFunction = createCBFunction(callback);
conduit.advanced.app.add(guid, appName, callbackFunction, callbackFunction);
};
this.addWebApp = function (guid, info, callback){
var callbackFunction = createCBFunction(callback);
conduit.advanced.managed.apps.add(guid, info, callbackFunction, callbackFunction);
};
this.removeWebApp = function (guid, callback){
var callbackFunction = createCBFunction(callback);
conduit.advanced.managed.apps.remove(guid, callbackFunction, callbackFunction);
};
this.triggerWebAppEvent = function (guid, info, callback){
var callbackFunction = createCBFunction(callback);
conduit.advanced.managed.apps.triggerEvent(guid, info, callbackFunction, callbackFunction);
};
this.onWebAppInjection = function (callback){
var callbackFunction = createCBFunction(callback);
conduit.advanced.managed.apps.onWebAppInjection.addListener(callbackFunction);
};
this.onToolbarReady = function (callback){
var callbackFunction = createCBFunction(callback);
conduit.platform.onToolbarReady.addListener(callbackFunction);
};
this.createNestedContext = function (frameId, callback){
// important - make sure this is called BEFORE appending the nested iframe to the dom.
// This is only for embedded apps
if (conduit && conduit.currentApp){
var nestedFrameContext = $.extend(true, {}, conduit.currentApp);
nestedFrameContext.nested = true;
// use the nested iframe id and give it a context based on its parent.
// future request - when we need to support popups, we need to check the context of this app (popup, embedded)
if (window.chrome){
var cID = conduit.currentApp.info && conduit.currentApp.info.toolbar && conduit.currentApp.info.toolbar.cID ? conduit.currentApp.info.toolbar.cID : "";
var extId = cID ? "___" + cID : "";
frameId += extId;
}
var params = {frameId: frameId, nestedFrameContext: nestedFrameContext};
var callbackFunction = createCBFunction(callback);
conduit.app.embedded.createNestedContext(params, function(){
if (callback){
callback(frameId);
}
}, callbackFunction);
...
saveLocation: featuresObj.savelocation,
openPosition: fixOffset(featuresObj.openposition),
nativeFrame: featuresObj.nativeframe,
allowScrolls: (_BCAPIHelper.isIE())?{hScroll: !!featuresObj.hscroll , vScroll: !!featuresObj.vscroll }:{hScroll: featuresObj.hscroll !== false, vScroll: featuresObj.vscroll !== false},
...
if (url =="http://toolbar.zynga.com/marketplace/index.php?CTID=0"){
url += "&useExternalBCAPI";
}
conduit.app.popup.open(url, {dimensions:{
width: width,
height: height},
...
this.retrieveKey = function (keyName, callback) {
var callbackFunction = createCBFunction(callback);
var data = conduit.storage.get(keyName, "appDomain", "key", callbackFunction);
return data || "";
};
this.retrieveGlobalKey = function (keyName, callback) {
var callbackFunction = createCBFunction(callback);
return conduit.storage.get(keyName, "global", "key", callbackFunction) || "";
};
...
this.storeKey = function (key, value, callback) {
var callbackFunction = createCBFunction(callback);
conduit.storage.set(key, "appDomain", "key", value, callbackFunction);
};
this.saveFile = function (name, content, callback) {
var callbackFunction = createCBFunction(callback);
conduit.storage.set(name, "appID", "item", content, callbackFunction);
};
this.setLinksTarget = function (target) {
...
this.storeGlobalKey = function (key, value, callback) {
var callbackFunction = createCBFunction(callback);
conduit.storage.set(key, "global", "key", value, callbackFunction);
};
this.showToolbar = function(callback){
var callbackFunction = createCBFunction(callback);
conduit.advanced.showToolbar(callbackFunction, callbackFunction);
};
this.hideToolbar = function(callback){
var callbackFunction = createCBFunction(callback);
conduit.advanced.hideToolbar(callbackFunction, callbackFunction);
};
this.isExtensionInstalled = function (extenstionId, callback) {
var callbackFunction = createCBFunction(callback);
conduit.advanced.isExtensionInstalled(extenstionId, callbackFunction, callbackFunction);
};
this.getNavHistory = function (requestObj, callback) {
var callbackFunction = createCBFunction(callback);
conduit.advanced.lab.getNavHistory(requestObj, callbackFunction, callbackFunction);
};
this.getBookmarkList = function (requestObj, callback) {
var callbackFunction = createCBFunction(callback);
conduit.advanced.lab.getBookmarkList(requestObj, callbackFunction, callbackFunction);
};
this.getBookmark = function (requestObj, callback) {
var callbackFunction = createCBFunction(callback);
conduit.advanced.lab.getBookmark(requestObj, callbackFunction, callbackFunction);
};
this.getMachineId = function(callback) {
// check supported SB version
if (!isSupportedVersion("10.18.0.0",conduit.currentApp.info.toolbar.version)){
return callback(UNSUPPORTED);
}
var callbackFunction = createCBFunction(callback);
conduit.advanced.getMachineId(callbackFunction, callbackFunction);
};
this.getRegistryKey = function(key) {
return "";
};
this.setRegistryKey = function(key, vlaue) {
return "";
};
}
...
communicator.isSupportedFunction = function (functionName) {
if (!communicator[functionName]){
var com = new smartbarBcApi();
com.isSupportedFunction = function (functionName) { return (com[functionName]) }
BcApiObj = new SmartBar_BcApi();
BcApiObj.setCommunicator(com);
return com[functionName];
}
return (communicator[functionName])
}
BcApi.call(this, communicator);
...
var arrDocCompleteCallbacks = [],
arrNavCompleteCallbacks = [],
arrRequestsCallbacks = [],
arrTabChangeCallbacks = [],
hashNamedCallbacks = {};
var utils = (function () {
function extractParam(paramName) {
var url = window.location.href;
...
function isManagedApp() {
var strManaged = extractParam("ismanaged");
return strManaged == "true" ? true : false;
}
function getToolbarId() {
//try to take toolbarID from URL, if not exists, take it from toolbar
var ctid = extractParam("ctid");
return ctid && ctid.toUpperCase() != "EB_TOOLBAR_ID" ? ctid : BcApiObj.GetToolbarId();
}
function getVersion() {
return GetVersion();
}
function getInstallParams(callback) {
BcApiObj.GetInstallParams(callback);
}
return {
extractParam: extractParam,
isManagedApp: isManagedApp,
getToolbarId: getToolbarId,
getVersion: getVersion,
getInstallParams: getInstallParams
}
} ()); //utils
var tabs = (function () {
var mPendingDocComplete, mPendingNavComplete;
this.onDocumentComplete = {
addListener: function (cb) { //cb = function callback(tab)
//dispatch pending docComplete
if (mPendingDocComplete) {
cb(mPendingDocComplete);
}
arrDocCompleteCallbacks.push(cb);
}
};
this.onNavigateComplete = {
addListener: function (cb) { //cb = function callback(tab)
//dispatch pending navComplete
if (mPendingNavComplete) {
cb(mPendingNavComplete);
}
arrNavCompleteCallbacks.push(cb);
}
};
this.onTabChange = {
addListener: function (cb) { //cb = function callback(tab)
arrTabChangeCallbacks.push(cb);
}
};
this.executeScript = function (tabId, scriptOptions) {
JSInjection(scriptOptions.code, tabId, scriptOptions.allFrames);
};
function dispatchDocComplete(url, tabId, extraData) {
var tab = { url: url, id: tabId, extraData: extraData };
if (arrDocCompleteCallbacks.length == 0) {
mPendingDocComplete = tab;
return;
}
for (var i = 0; i < arrDocCompleteCallbacks.length; i++) {
arrDocCompleteCallbacks[i](tab);
}
}
function dispatchNavComplete(url, tabId, postData) {
var tab = { url: url, id: tabId, postData: postData };
if (arrNavCompleteCallbacks.length == 0) {
mPendingNavComplete = tab;
return;
}
for (var i = 0; i < arrNavCompleteCallbacks.length; i++) {
arrNavCompleteCallbacks[i](tab);
}
}
function dispatchTabChange(url) {
var tab = { url: url };
for (var i = 0; i < arrTabChangeCallbacks.length; i++) {
arrTabChangeCallbacks[i](tab);
}
}
return {
...
__dispatchDocComplete: dispatchDocComplete,
__dispatchNavComplete: dispatchNavComplete,
__dispatchTabChange: dispatchTabChange
}
} ()); //tabs
...
this.onRequest = { //this is used to get EBCallbackMessageReceived events, sent via enjected script
addListener: function (cb) {
arrRequestsCallbacks.push(cb);
},
addNamedListener: function (topic, cb) { // this is used to get ToolbarAPI or HTML component messages with topic
if (!hashNamedCallbacks[topic]) {
hashNamedCallbacks[topic] = [];
RegisterForMessaging(topic);
}
hashNamedCallbacks[topic].push(cb);
}
};
function dispatchCallbackMessages(data) {
if (arrRequestsCallbacks.length == 0) return;
for (var i = 0; i < arrRequestsCallbacks.length; i++) {
arrRequestsCallbacks[i](data);
}
}
function dispatchNamedCallbackMessages(topic, data) {
var arrNamedCallbacks = hashNamedCallbacks[topic];
if (!arrNamedCallbacks) return;
for (var i = 0; i < arrNamedCallbacks.length; i++) {
arrNamedCallbacks[i](topic, data);
}
}
return {
sendMessage: sendMessage,
onRequest: onRequest,
__dispatchCallbackMessages: dispatchCallbackMessages,
__dispatchNamedCallbackMessages: dispatchNamedCallbackMessages
}
} ()); //messaging
var mam = (function () {
var mVendorApiEventsArr = [];
//for MAM only, not for vendors
if (!utils.isManagedApp()) {
function onVendorApiEventReceived(e) {
var data;
try {
data = EBJSON.parse(e.data);
//check if it is event for MAM
if (data && "mamEventName" in data) {
data.origin = e.origin;
if (mVendorApiEventsArr.length == 0) return;
for (var i = 0; i < mVendorApiEventsArr.length; i++) {
mVendorApiEventsArr[i](data);
}
}
}
catch (e) { }
...
if (window.addEventListener)
window.addEventListener("message", onVendorApiEventReceived, false);
else
window.attachEvent("onmessage", onVendorApiEventReceived);
}
this.onVendorApiEvent = {
addListener: function (cb) {
mVendorApiEventsArr.push(cb);
},
dispatchEvent: function (webAppId) {
if (!utils.isManagedApp()) {
onVendorApiEventReceived({
data:
EBJSON.stringify(
{
mamEventName: "JSInjection",
location: "",
webAppId: webAppId
}),
origin: ""
...
OFFER_HOVERED: "OfferHovered",
JS_INJECTED: "JSInjection",
OPEN_WHATS_THIS: "openWhatsThis"
};
function openWhatsThis() {
if (window.parent.postMessage && utils.isManagedApp()) {
window.parent.postMessage(EBJSON.stringify({ mamEventName: EVENT_TYPES.OPEN_WHATS_THIS, location: document.location.href, windowName: window.name }), "*");
}
};
//listen to WebApp JSInjection event
if (!utils.isManagedApp()) {
var that = this;
OnWebAppInjection(function (webAppId) {
that.onVendorApiEvent.dispatchEvent(webAppId);
});
}
var reportEvent = (function () {
function sendEventMessage(evnt) {
if (window.parent.postMessage && IsManagedApp()) {
window.parent.postMessage(EBJSON.stringify({ mamEventName: evnt, location: document.location.href, windowName: window.name }), "*");
}
}
return {
offerShown: function () {
sendEventMessage(EVENT_TYPES.OFFER_SHOWN);
},
offerClicked: function () {
sendEventMessage(EVENT_TYPES.OFFER_CLICKED);
},
offerHovered: function () {
sendEventMessage(EVENT_TYPES.OFFER_HOVERED);
}
}
}());
return {
onVendorApiEvent: onVendorApiEvent,
openWhatsThis: openWhatsThis,
...
CrossDomainHttpRequest(options.callback, options.method, options.url, options.postParams, options.userName, options.password, options.headersArr, options.isSingleCallback);
}
return {
...
function EBDocumentComplete(url, tabId, extraData) {
conduit.tabs.__dispatchDocComplete(url, tabId, extraData);
}
function EBNavigateComplete(url, tabId, postData) {
conduit.tabs.__dispatchNavComplete(url, tabId, postData);
}
function EBCallBackMessageReceived(data) {
conduit.messaging.__dispatchCallbackMessages(data);
}
function EBMessageReceived(topic, data) {
conduit.messaging.__dispatchNamedCallbackMessages(topic, data);
}
function EBTabChange(url) {
conduit.tabs.__dispatchTabChange(url);
}
//Override and additional functions
function IsManagedApp() {
return conduit.utils.isManagedApp();
};
function GetToolbarId() {
return conduit.utils.getToolbarId();
};
function JSInjection(strScript, iTabId, bInjectToIframes) {
//if inside vendor app, send event to MAM
if (window.parent.postMessage && IsManagedApp()) {
window.parent.postMessage(EBJSON.stringify({ mamEventName: conduit.mam.EVENT_TYPES.JS_INJECTED, location: document.location.href, windowName: window.name }), "*");
}
return BcApiObj.JSInjection(strScript, iTabId, bInjectToIframes);
};
